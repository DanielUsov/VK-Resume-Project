...
const FAKE_RESPONSE = JSON.stringify([
	...
	},
	{
		...
		explanation:
			'Магистратура, 1900 г. Диплом по тематике аналитических систем для бизнес-интеллидженс.',
		type: 'edu',
	},
]);
...
export class InfoCollection {
	...
	fetchData() {
		...
		const responseData = JSON.parse(FAKE_RESPONSE);
		this.data = responseData.map(
			(item) => new Info(item.title, item.explanation, item.type, item.bullets)
		);
	}
